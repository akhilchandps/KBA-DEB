...
const AddResult = () => {
  ...
  const getOneStudent = async (id) => {
    if (!id) return; 
    try {
      const res = await fetch(`http://127.0.0.1:5000/getStudent/${id}`, {
        method: "GET",
        credentials: "include",
      });
      if (res.ok) {
  ...
  const getSubjects =async(id)=>{
   ...
   const response = await fetch(`http://127.0.0.1:5000/getSubjectClass/${id}`,{
      method:"GET",
      credentials:"include"
   })
  ...
  const handleResult = async (e) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:5000/addResult`, {
        method: "POST",
        headers: {
